...
select
	...
	@cdatetime_first = '2016.02.06 05:00', @cdatetime_last = '2016.02.22 21:00'	
	...
	select top 1 *, space(16) as cdatetime
	into #nt_st_deals
	from nt_st_deals
...
-- select * from nt_st_chart where idn = 63158
	...
	exec ntp_st_MakeDealsAllDays_v02
		...
		-- прочие параметры стратегии
		@param_volume = 10000,
		@param_IsOnlyOneActiveDeal = 0, -- 1 = только одна открытая позиция, 0 = неограниченное число открытых позиций (но в рамках одного временного периода - только одна)
	...
	from #nt_st_deals
	--where deal_profit < 0
	--  and deal_direction = 2
	--order by idn --where param_DealTimeInMinutesFirst = 300
	
	--select * from ntAverageValuesResults where idn = 3448
