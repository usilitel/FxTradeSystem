...
alter PROCEDURE nnpSelect_nnImportCurrent (@pParamsIdentifyer VARCHAR(50))
AS BEGIN 
-- процедура выводит текущие входные данные
...
SET NOCOUNT ON
	...
	select @strQuery1 = 'CREATE TABLE #t_nnImportCurrent (idn int identity(1,1),' + @str_FieldList1 + ') ON [PRIMARY]'
	select @strQuery2 = 'insert into #t_nnImportCurrent (' + @str_FieldList2 + ') select ' + @str_FieldList3 + ' from nnImportCurrent WITH(NOLOCK)  where ParamsIdentifyer = ''' + @pParamsIdentifyer + ''' order by idn'
	select @strQuery3 = 'select ' + @str_FieldList2 + ' from #t_nnImportCurrent order by idn'



	-- выполняем запросы
	If object_ID('tempdb..#t_nnImportCurrent') Is not Null drop table #t_nnImportCurrent
